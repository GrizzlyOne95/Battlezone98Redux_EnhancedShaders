...
void main()
{
	...
	vec3 viewNormal = safe_normalize(normalTex.xyz * tbn);
#else
	...
	vec3 viewReflect = reflect(safe_normalize(viewPos), viewNormal);
#endif
	...
	float grey = oColor.x * 0.3 + oColor.y * 0.6 + oColor.z * 0.1;
	oColor.x = grey;
	oColor.y = grey;
	oColor.z = grey;
#endif
