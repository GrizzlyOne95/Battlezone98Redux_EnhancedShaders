...
)
{
	...
	float4 dTexSample = diffuseMap.Sample(diffuseSam, uv);
	...
	float3 nTex = normalMap.Sample(normalSam, uv).xyz * 2.0 - 1.0;
	...
	float3 specTex = srgb_to_linear(specularMap.Sample(specularSam, uv).xyz);
	float glossTex = specularMap.Sample(specularSam, uv).a;
	float3 emissiveTex = srgb_to_linear(emissiveMap.Sample(emissiveSam, uv).xyz);
